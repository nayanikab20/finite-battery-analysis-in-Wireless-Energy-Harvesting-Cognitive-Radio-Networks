...
alpha= 0.5;
T =1;% (1/1)*10^-9;
threshold1 = db2pow(inf);   % for battery for SS
...
for th = 1:3
    ...
    for interference = 1:41
        ...
        for i = 1:size
            ...
            phs(i) = min(Phs(i), threshold1);            
            p = eta*alpha/(1-alpha);
            
            %z1(i)=PU_tx*(f11(i)+f12(i)+f13(i));
            %z2(i) = PU_tx(f21(i)+f22(i)+f23(i));
...
%figure
semilogy(-20:1:20, P_out_sim(1,:), 'r')
hold on;
semilogy(-20:1:20, P_out_sim(2,:), 'g')
semilogy(-20:1:20, P_out_sim(3,:), 'b')
xlabel('P_I (dBW)')
ylabel('P_out')
legend('gamma th= -10', 'gamma th = -5', 'gamma th = 0')
